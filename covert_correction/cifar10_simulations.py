...
from pack.fp4 import fp4_121_bin_edges
# Math
import math
import numpy as np
...
def run_experiment(args):
    ...
    fp4_bin_centers,fp4_bin_edges,fp4_dict = fp4_121_bin_edges()
    ...
    def step(X, y, error, s):
        ...
        if epoch == 0:
            for idx in range(len(grads)):
                ...
                shape = np.shape(temp)
                quantized_gradient = np.reshape(fp4_bin_centers[np.searchsorted(fp4_bin_edges, temp.flatten())], shape)
            ...
            for idx in range(len(grads)):
                ...
                temp += 0.5*error[idx]
                grad_plus_error.append(temp.flatten())
                quantized_gradient = np.reshape(fp4_bin_centers[np.searchsorted(fp4_bin_edges, temp.flatten()*scalar[idx])]/scalar[idx], shape)
                quantized_np.append(quantized_gradient)
                error[idx] = temp - quantized_gradient
            ...
            if (batches + 1) >= batch_total:
                ...
                if epoch % 5 == 4:
                    for l in range(len(avr_grads)):
                        ...
                        x = fit_params[0]
                        scalar[l] = (-1.87625755 * x) + (2.87957847 * x**2) + (0.11091502 * x**3) + (-1.13110354 * x**4) + (0.35043158 * x**5) + 0.31690492
                        if scalar[l] < 0.0009:
                            scalar[l] = 0.0009/fit_params[2]
                        else:
                            scalar[l] /= fit_params[2]
                        #print("epoch ",epoch,"layer95 scalar:",scalar[95])
